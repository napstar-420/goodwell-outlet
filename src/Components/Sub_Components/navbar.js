...
const Navbar = () => {
  ...
  return (
    <nav className="">
      <ul
        id="nav_top"
        className="hidden lg:flex justify-around items-center  bg-[#5E5E5E] text-white relative z-50 top-[3.2rem]"
      >
        <li> Free Shipping on order above 150$</li>
        <li className="flex items-center hover:underline cursor-pointer">
          <BsTelephoneFill className="mr-1" /> +1-123-456-789
        </li>
        <li className="flex items-center hover:underline cursor-pointer">
          <IoIosMail className="text-lg mr-1" /> Official@goodwilloutlet.com
        </li>
        <li className="flex items-center hover:underline cursor-pointer">
          <MdLocationOn className="text-lg mr-1" /> California / USA
        </li>
      </ul>
      <ul
        id="nav_bottom"
        className={`bg-[#303030] py-2 px-4 flex lg:flex-row flex-col lg:items-center lg:justify-between w-[100%] z-50 fixed top-0  ${
          navSelected ? `h-screen` : `h-[3.5rem]`
        } overflow-hidden transition-all lg:h-max`}
      >
        <li className="flex items-center">
          <GiHamburgerMenu
            className="lg:hidden text-white text-3xl mr-2 hover:opacity-50"
            onClick={() => setNavSelected(!navSelected)}
          />
          <Link to='/' className="text-3xl font-bold text-white">Goodwill Outlet</Link>
        </li>
        <ul className="flex text-white lg:flex-row flex-col my-2 lg:my-0">
          <li className="border-[#5E5E5E] border-2 p-1 rounded-full flex items-center justify-between lg:hidden  mt-2 lg:mt-0">
            <input
              type="text"
              className="bg-[#303030] px-1 rounded-full mr-1 placeholder:text-white focus:outline-none focus:bg-[#5E5E5E] text-white focus:border-none"
          ...
          <li className="text-lg lg:text-base px-1 py-2 lg:py-0 hover:opacity-50 cursor-pointer underline underline-offset-4 lg:hidden">
            FAVORITES
          </li>
          <li className="text-lg lg:text-base px-1 py-2 lg:py-0 hover:opacity-50 cursor-pointer underline underline-offset-4 lg:hidden">
            MY CART
          ...
          <button
            className="text-lg lg:text-base px-2 py-2 lg:py-0 hover:opacity-50 cursor-pointer w-max mx-auto rounded-full border-2 border-white lg:hidden"
            onClick={() => setNavSelected(false)}
          >
            <ImCross className="text-center" />
          </button>
          <li className="text-lg lg:text-base px-1 py-2 lg:py-0 hover:opacity-50 cursor-pointer text-center lg:hidden">
            NAPSTAR &copy; | 2022
          </li>
        </ul>
